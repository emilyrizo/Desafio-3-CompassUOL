...
interface ProductListProps {
  itemsPerPage: string; // Recebendo a prop
}
...
const ProductList = ({ itemsPerPage }: ProductListProps) => {
  ...
  // Limita o número de produtos baseado em itemsPerPage
  const displayedProducts = products.slice(0, Number(itemsPerPage)); // Converte itemsPerPage para número

  return (
    <div className="product-container">
      <ul className="product-list">
        {displayedProducts.map((product) => (
          <li className="product-card" key={product.id}> {/* Certifique-se que key está correto */}
            <div className="product-image-wrapper">
              <a href="">
                <img src={product.image_link} alt={product.name} className="product-image" />
                {product.is_new && <span className="tag-new">New</span>}
